...
const CoinModal = ({ open, onClose, children, width = 500 }) => {
    ...
    for (let i = 0; i < tokenCount.length; i++) {
      for (let j = 0; j < tokenCount[i].length; j++) {
        if (tokenCount[i] && tokenCount[i][j] && Number(tokenCount[i][j]) > 0) {
          ...
          const tokenId = `${i}_${j}`;
          const url =
            playerUrls[i] && playerUrls[i][j] ? playerUrls[i][j] : null;
  ...
  const targetNetwork =
    selectedNetwork === "avalanche" ? "sepolia" : "avalanche";
  ...
  const handleNFTBridgeTransfer = async () => {
    ...
    try {
      ...
      console.log(
        "Getting last token ID for playerType:",
        playerType,
        "uriIndex:",
        uriIndex
      );
      ...
      if (error?.message?.includes("User rejected")) {
        ...
        alert(
          `NFT Bridge transfer failed: ${error?.message || "Unknown error"}`
        );
      }
    }
  };
    ...
    >
      ...
      >
            ...
            <div style={{ display: "flex", gap: 16 }}>
              <button
                ...
                style={{
                  ...
                  padding: "18px 25px",
                  border: `3px solid ${
                    transferType === "coins" ? "#fde047" : "#666"
                  }`,
                  background:
                    transferType === "coins" ? "#fde047" : "transparent",
              ...
              <button
                ...
                style={{
                  ...
                  padding: "18px 25px",
                  border: `3px solid ${
                    transferType === "nft" ? "#fde047" : "#666"
                  }`,
                  background:
                    transferType === "nft" ? "#fde047" : "transparent",
        ...
        >
          ...
          >
            Current Network :{" "}
            {networks[selectedNetwork]?.name || "Unknown Network"}
          </div>
          ...
          >
            ...
            >
              <span
                style={{ color: networks[selectedNetwork]?.color || "#fde047" }}
              >
              ...
              <span
                style={{ color: networks[targetNetwork]?.color || "#fde047" }}
              >
              ...
              {!address ? (
                ...
                <>
                  ...
                  {selectedNFT && (
                    ...
                    >
                      {(() => {
                        const selectedPlayer = nftPlayers.find(
                          (nft) => nft.id === selectedNFT
                        );
          ...
          <button
            ...
            disabled={
              ...
              (transferType === "nft" &&
                (!address || isLoadingNFTs || nftPlayers.length === 0))
            }
            style={{
              width: "100%",
              padding: "18px 25px",
          ...
          >
            {isTransferring
              ...
              : transferType === "coins"
              ? `Transfer ${coinAmount || "0"} ${
                  coinUnits.find((unit) => unit.value === coinUnit)?.label
                } via CCIP`
              : transferType === "nft" &&
                (!address || isLoadingNFTs || nftPlayers.length === 0)
              ? "No NFTs Available"
              : `Transfer ${
                  nftPlayers.find((nft) => nft.id === selectedNFT)?.name ||
                  "NFT Player"
                } via CCIP`}
          </button>
